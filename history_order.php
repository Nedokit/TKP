<?php
	...
	$temp = intable_search_item("SELECT * FROM `history_orders` WHERE `id`=".$_GET['order_id']);
...
<!--Контент-->
				<div style="padding-top: 30px; padding-bottom: 30px;" class="conteiner">
					...
					<?php
						if($_SESSION['user_info']['id'] != $temp['exe_id'] && intable_count_items("SELECT `id` FROM `history_orders` WHERE `id`=".$_GET['order_id']." AND `user_id`=".$_SESSION['user_info']['id']." AND `exe_id`<>-1") == 0)
							if($temp_time > 0)
								echo('
